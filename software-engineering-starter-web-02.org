...
As a continuation of [[./software-engineering-starter-web-02.org][Starter Web App]], we'll be making our server respond with
...
** announcing the html
   ...
   for now to set headers, and this is what we need to do to indicate we're
   sending HTML back:
   ...
   server running. Once that's done, you can start the server back up with =yarn
   start=.
