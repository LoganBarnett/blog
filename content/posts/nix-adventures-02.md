...
aliases = ["/nix-adventures-02.html"]
categories = ["nix"]
draft = false
+++
...
- [Today's adventure: Building a Suite of Machine Learning Servers](#today-s-adventure-building-a-suite-of-machine-learning-servers)
    - [Building on macOS (LocalAI)](#building-on-macos--localai)
    - [Building on macOS (stable-diffusion-webui)](#building-on-macos--stable-diffusion-webui)
...
setup their own servers with this stuff.  [LocalAI](https://localai.io) looks good for this.  Cool
...
I don't <span class="underline">need</span> this to be on macOS, but it's easier for me if I can.
...
platforms.  From my reading, this is an [apparent problem with Nix Flakes](https://github.com/NixOS/nix/issues/3843), and
...
platform?".  The latter two are attempts to address that - [both with problems](https://ayats.org/blog/no-flake-utils/)
...
[NixOS wiki](https://nixos.wiki/wiki/Flakes) seems to be the most authoritative documentation I can find, and it
...
[devenv.sh](https://devenv.sh/guides/using-with-flakes/#modifying-your-flakenix-file) has an example though:
...
[an example](https://www.softwarefactory-project.io/reproducible-shell-environments-via-nix-flakes.html) where they use `pkgs.mkShell`.  Oh so it's a non-package in the
...
[comment](https://github.com/NixOS/nixpkgs/issues/269582#issuecomment-1909046618) has the derivation!  I'd read this before and that was not present.
...
In some [arbitrary sample](https://github.com/NixOS/nixpkgs/blob/master/pkgs/applications/audio/airwave/default.nix) I found `$out` used in `postInstall`, which looks
...
I stumbled across [Flake my life - how do nix flakes work?](https://dev.to/dooygoy/flake-my-life-how-do-nix-flakes-work-2foe) which captures another
...
Which I already knew and still have <span class="underline">no idea what should go into the `path`
field</span>.
...
[exactly what I want and documented as such kind of](https://nixos.org/guides/nix-pills/callpackage-design-pattern).
...
package](https://github.com/NixOS/nixpkgs/blob/7ba3caea70556bb6691f0ed0792c2ac89393018b/pkgs/applications/editors/neovim/neovide/default.nix#L86-L102) has this:
...
[other frameworks are mentioned](https://github.com/mudler/LocalAI/issues/1560#issuecomment-1890859859).  Those should be added to the list.  I've copied
...
From that same issue, I see a [comment](https://github.com/mudler/LocalAI/issues/1560#issuecomment-1912502699) mentioning that `falcon-ggml` is
deprecated and there are means to [exclude](https://localai.io/basics/build/#build-only-a-single-backend) that "backend" from the build, or at
least only build the ones I want.  I can't find mention of `falcon-ggml` in the
derivation, but from reading the [linked issue](https://github.com/mudler/LocalAI/issues/1126), I can find that the thing being
...
running `make build`.  I found an [example](https://github.com/mudler/LocalAI/blob/16cebf0390927929380961cbbd8397d1c8008afd/Dockerfile#L16) which shows that backends are
...
This [issue](https://github.com/NixOS/nix/issues/705) shows I should be able to add `darwin.security_tool` as a dependency
...
The best I've been able to find about this is [nixpkgs#47676](https://github.com/NixOS/nixpkgs/pull/47676) wherein the path is
...
up in a `Makefile` - `Makefiles` <span class="underline">require</span> tabs, because reasons.  I don't think
...
Which does <span class="underline">not</span> use the colon syntax.  It's space-separated directories.  Let's
do this:
...
patch from the [LocalAI#1560](https://github.com/mudler/LocalAI/issues/1560#issuecomment-1890859859) recommendation (this ticket is getting a lot of
...
there's a direct mention of this in [LocalAI#713](https://github.com/mudler/LocalAI/issues/713).  One of the recommendations is
...
Let's go back to the "try just one thing" and see if we can build <span class="underline">one</span> backend.
...
So setting this value to <span class="underline">known working values</span> causes a problem here.  I don't
...
Hey [look at that](https://github.com/virchau13/automatic1111-webui-nix).
...
`stable-diffusion-webui`, let's move that to the [third Nix adventure]({{< relref "nix-adventures-03" >}}) in the
series.
