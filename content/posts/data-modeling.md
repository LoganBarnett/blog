...
## What's an Edge Case? {#what-s-an-edge-case}

First we need to come to an agreement on what an edge case is.  [Wikipedia has a
decent edge case definition](https://en.wikipedia.org/wiki/Edge_case), but let's take our own spin on it:

> An edge case requires you to heap code upon your program to address
> unanticipated needs.


### Edge Case Example {#edge-case-example}
...
### Expanding Edge Cases Further {#expanding-edge-cases-further}
...
it is we were <span class="underline">forced</span> to make code changes in the application. This might seem
like an obvious thing in the face of a new requirement, but consider that <span class="underline">every
place a Moderator could do something, code must be changed</span>. In a large
...
application.  Whether you go through this exercise or not, <span class="underline">your application has
a data model</span>. Having an explicit model is always going to serve you better than
an implicit one (the one in your head).


### Data Modeling Users {#data-modeling-users}

A simple means of modeling data involves capturing behavior and representing it
<span class="underline">purely as data</span> rather than inflicting code constructs upon it. Using our
...
2.  Moderators can perform <span class="underline">some</span> special operations.
3.  Moderators can perform normal operations, <span class="underline">sometimes</span> absent of those
...
that is logged in is still a user. Perhaps not a <span class="underline">registered</span> user, but a user
...
also something we'll revisit). Additionally, a `Role` can represent <span class="underline">any future
type of user</span>. No extra code needed! Just add some data. Realistically we may
need to introduce <span class="underline">some</span> code, but it will be very minimal in comparison to our
example so far.
...
that would be missing the point. We need to decouple a role from a <span class="underline">permission</span>.
...
We then stop checking `session`, and instead we assume we <span class="underline">always</span> have a user,
...
### Real World Data Modeling {#real-world-data-modeling}

What's seen in [Data Modeling Users](#data-modeling-users) is a real world data model that gets used
...
[Double entry bookkeeping](https://en.wikipedia.org/wiki/Double-entry_bookkeeping) is a data model that involves keeping a series of
...
Our beloved [Git](https://en.wikipedia.org/wiki/Git) uses patches, commits (which hold patches), and refs (which hold
...
### Immutable Data Models - Events {#immutable-data-models-events}
...
We need a date that corresponds to the event to avoid [Peril: Insertion Order](#peril-insertion-order).
...
### LEGO: Using Simple Models to Achieve Complexity {#lego-using-simple-models-to-achieve-complexity}
...
### Peril: Insertion Order {#peril-insertion-order}


### Peril: Mutation {#peril-mutation}


### Peril: Anonymous Edits {#peril-anonymous-edits}
