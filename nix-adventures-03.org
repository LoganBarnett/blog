...
Well, there isn't actually.  But there _should be a better way_ and I don't
think that's been a focus yet.

It takes about 50 minutes to build on my laptop.  I understand the complaint
about time but all I care about right now is _that it works_.
...
:PROPERTIES:
:CUSTOM_ID: adventure:-standing-up-a-=stable-diffusion-webui=-server-on-a-new-host--preparing-the-host--direct-builds--deploying-arbitrary-changes-to-the-host
:END:
...
[[https://github.com/serokell/deploy-rs][deploy-rs]].  It aims to allow me to deploy to any system I have SSH access to.
...
Surely they don't mean for me to run _my entire flake_?  And surely not from
...
[[https://github.com/nix-community/home-manager/issues/1642][home-manager#1642]] suggests that the =@inputs= could somehow be involved (even
...
configuration hoops that I shouldn't, or it _mandates_ that you cannot cross
compile.

It's a little infuriating that I can build an ISO somehow but not the _actual
image itself_.  If I just understood the tooling a little better this could be
different.
...
and just run =nixos-rebuild switch --flake '.#lithium'= and that _should_
...
this context from [[https://github.com/nix-community/nixos-generators][nixos-generators]]:
...
things, like I _should outright ignore the documentation and just go look in the
code_.  This bodes very ill for Nix, and I hope the values held there change
...
things, I _start_ with the documentation.
...
_why_, and it would've saved enormous spelunking on my part.  Folks should still
...
problems.  I was having similar, sporadic build issues found in [[https://github.com/NixOS/nixpkgs/issues/206213][nixpkgs#206213]]
...
Physical memory exists on a [[https://en.wikipedia.org/wiki/Bus_(computing)#Memory_bus][memory bus]].  This means data being written to or
...
but that it can't resolve to a system.  Strangely it will try _something_ - I
