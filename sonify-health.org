...
I built a tool I call [[https://github.com/LoganBarnett/sonify-health][sonify-health]] I use to assist with monitoring my home
network.  It can be used for monitoring all kinds of things passively.  I
...
tech is pretty crufty, but it mostly works and I'm the glue-on-retainer that
repairs it when it stops working.  At home I have a handful of physical machines
I keep up, but independently there are several dozen services I'm holding up
with it.  I'm approaching a low tier enterprise grade IT setup.
...
~top~ to see what's going on.  That doesn't work when thousands of entities are
...
But what happens when we _don't_ want to get woken up over some information
...
an example.  Now sometimes they have sounds being made to accompany some already
visual information.  A window popping up or something animating.  If you're old
enough, you might remember people tricking out their Windows 3.x or 95 setups
with this kind of behavior, and then nobody did it ever again.  We're not
talking about those kinds of sounds.
...
over again but that's because _everything is fine_.  What happens when it's not
...
hear bird song in a wilderness area, it's indication that a predator might be
...
But we know people are talking around us and thing are fine.  Then at some point
it goes from friendly conversation to hostile, and it enters our consciousness.
You weren't paying active attention to the conversation and yet a part of you
was paying enough attention to it to know when it deviated significantly.

The act of taking information and encoding it into audio (generally via
arbitrary sounds and not spoken words) is called [[https://en.wikipedia.org/wiki/Sonification][Sonification]].  It's a rather
...
exit code with arbitrary assignments to exit codes. You can also have it look at
...
Gradient notes work best with a normalized value (something 0.0 and 1.0).  You
indicate two different patches for the note, and indicate what the scaling lerp
strategy is (so it necessarily not be linear).  ~sonify-health~ can, live,
update the sound being played to correspond to something in the middle of the
two values, based on what the probe brought back.

Okay so that's a lot of abstract words.  Let's take a look at an example of the
UI.

[[file:./sonify-health-heartbeat-discrete-gateway-01.png]]
...
+ Patch :: This isn't in this part of the UI, but an important concept.  A patch
  ...
  uses.
+ Command :: This is just a ping to the gateway by IP.  It has an aggressive
  timeout and only sends a single ping.
+ Result mode :: We're going with exit code.  A successful ping exits with 0,
  and an unsuccessful ping will be non-zero.  This fits right in.
+ Playback :: This is set to ~clock~, which is short for wall clock.  Basically
  instead of being in a loop where the loop begins when the service, it occurs
  according to wall clock time - repeating at every Nth second during a minute.
...
+ Poll Interval :: How often we run the command.
+ Cycle :: The duration of one full audio cycle in seconds - when using wall
  clock it should be evenly divisible by 60.
+ Offset :: When in the cycle this heartbeat is played.  This is another field
  to help achieve temporal separation between multiple instances of
  ~sonify-health~.
+ Value :: This is the current value coming back from the command.  Right now
  it's failing, because I took the screenshot when away from home.  The value
  can be changed here, and the ~sonify-health~ will treat it as an override -
  good for trying out various sounds, or perhaps suppressing an alert.  The
  ~(live)~ indicates it is not overridden currently.
+ Notes :: This is where the heartbeat's sounds are declared.  A heartbeat will
  ...
  + Transition :: This one is set to Discrete, but it could also be Gradient.
    + Discrete :: Discrete notes play a particular patch based on discrete
      ...
      being success and 1+ being failure, we can setup two ranges (>= 0 and < 1
      for one, and >= 1 for the other).  The first patch is the success sound in
      the second is the error sound.
    + Gradient :: Gradient sounds are continuous range.  One or
      more transition points can be setup.  Each of these gets their own
...
with synthesizers before.  I haven't really, so I can't say that with much
authority.  While it's part of the data model, it's also kind of immaterial what
is exactly inside it.  The only thing that really matters is that the sound a
patch makes is entirely materialized from all of the fields that it uses.  This
allows us to transition between them.  We also have the ability to create an
override patch, where the patch follows another one, except for certain fields
it overrides.
...
At time of writing I have about 4 systems reporting in basic health, CPU, and
GPU information, and I want to add one more.  Those are kind of bland in a way.
While I want to make some more sophistication (like triage ticket "pressure"),
...
being tinkered with and I'm my own beta tester for all of it.  GPU usage has
been incredibly helpful.  I have several hosts contributing to LLM inference via
their GPUs.  One of them is a macOS host where GPU eviction is an ongoing
struggle.  Another problem is staggered usage.  So I've had long running
processes that delegate a lot of work to LLM inference, and when doing these
reports I expect the GPU of multiple systems to just be pegged.  Instead, it's
kind of a rapid oscillation.  This has informed me of problems I wouldn't have
otherwise known - I would've thought that was just the speed of inference.
Other times I've seen my ~garage-queue~ system that I'm using to spread
inferences across capable hosts wasn't distributing work correctly.  That's all
from just hearing things _while I'm doing something else_.  At some point I
realize what I'm hearing is not what I _should be hearing_.

CPU usage has informed me of runaway processes and on macOS, when a GPU based
job was evicted.
...
office first.  I could put on headphones and turn things down enough to not be
distraction easily enough, but it's also just enough friction to help me avoid
the distracting stuff.  In a similar vein, I don't have light music I'm
occasionally shopping for, because the same music just gets stale after a while.
...
I don't remember what each individual note represents, and from which sound or
speaker that it represents.  While I'm not totally lost when something is off,
I'm not really instantly saying "Oh looks like VPN is down" unless it's
triangulating with other information (e.g. I just hit a public website 3 seconds
ago so I know it's not my Internet connection that it's notifying me about,
which rules out which the chirp is the error chirp).  For some kinds of sounds,
I don't think I could reliably tell you between a 50% CPU utilization and 65%.
But for major steps I think I can tell fairly well.

**** A Strange Draw

If I'm being intellectually honest, I feel like there is a cool factor a bit.
It gives me a feel that I'm more than just sitting in a busy office where the
...
It also feels like I'm keeping the habit going where the sounds remain part of
this pattern my mind just expects to be present, and I feel like I notice it
...
from remote ~sonify-health~ instances so physical presence isn't required for
remote hosts.  This might involve more than just slurping in the data - I might
...
waveform.  It'll be sending over patch data and the state of reach heartbeat
poll.

I'd also like a _lot_ more built in sounds and examples.  I'll try to generalize
...
currently arguably took _more_ of my time to build than ~sonify-health~'s
